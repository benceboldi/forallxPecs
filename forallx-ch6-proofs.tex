...
Gyakran a logikai bizonyításokat \emph{derivációnak} nevezzük. Így $\script{A}\vdash\script{B}$ úgy is olvasható, hogy `\script{B} deriválható \script{A}-ból.'
...
A \define{tétel} egy mondat, ami deriválható premisszák nélkül, például \script{T} egy tétel akkor és csak akkor, ha $\vdash\script{T}$.
...
Két mondat \script{A} és \script{B} \define{bizonyíthatóan ekvivalens} akkor és csak akkor, ha mindkettő deriválható a másikból, például $\script{A}\vdash\script{B}$ and $\script{B}\vdash\script{A}$
...
Egy mondatkészlet $\{\script{A}_1,\script{A}_2,\ldots\}$ \define{bizonyíthatóan inkonzisztens} akkor és csak akkor, ha egy ellentmondás deriválható belőle, például valamely mondatra \script{B}, $\{\script{A}_1,\script{A}_2,\ldots\}\vdash\script{B}$ és $\{\script{A}_1,\script{A}_2,\ldots\}\vdash\enot \script{B}$.
